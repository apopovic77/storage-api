...
2. **Install dependencies** – Ensure the target project can run `echo 'No build required'` successfully.
...
6. **Test release** – Run `.devops/scripts/release.sh --no-build` (or full build) to fast-forward `main` and trigger the deploy workflow. Verify the GitHub Action can SSH to the server and publish files to `/var/www/storage-api`.
